...
public class BLPurchase extends Purchase implements IPurchase {
    ...
    public double getIGV(String idpurchase){
        ...
        try {
            ...
            xquery = "SELECT registrado FROM logistica_compra WHERE compra_id = ?;";
            String register = new Connect().ExecuteQuery(xquery, new Object[]{ idpurchase }, "registrado").toString();
        ...
        try {
            ...
            while(rs.next()){
                ...
                if(obj.getDiscount() != 0){
                    discount = ((obj.getPrice()*obj.getDiscount())/100);
                    obj.setPrice(obj.getPrice() - discount);
                    obj.setDiscounttotal(discount);
                }
                
                if(obj.getPerception() != 0){
                    obj.setPerception(((obj.getPrice() * obj.getPerception())/100));
